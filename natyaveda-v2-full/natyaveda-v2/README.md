...
# Open interactive shell (CPU)
docker compose --profile cpu run --rm natyaveda-cpu

# Open interactive shell (GPU)
docker compose --profile gpu run --rm natyaveda-gpu

# Verify setup in container
docker compose --profile cpu run --rm setup-cpu
docker compose --profile gpu run --rm setup-gpu

# Run full pipeline in Docker (extract -> build_splits -> train -> evaluate)
docker compose --profile cpu run --rm pipeline-cpu
docker compose --profile gpu run --rm pipeline-gpu

# Single-video inference
docker compose --profile gpu run --rm infer-gpu
docker compose --profile cpu run --rm infer-cpu
```

PowerShell note:
```powershell
# If you prefer docker run directly
docker build -t natyaveda:cpu --build-arg NATYAVEDA_INSTALL_MODE=cpu .
docker run --rm -it -v ${PWD}:/app natyaveda:cpu bash
```
