...
class DeviceManager:
    ...
    def __init__(
        ...
        requested_pose_model: str | None = None,
    ) -> None:
        self.force_cpu = force_cpu
        self.force_device = force_device
        self.requested_pose_model = requested_pose_model
    ...
    def _detect_and_configure(self) -> DeviceConfig:
        ...
        # Optional explicit pose model override from caller.
        if self.requested_pose_model:
            req = self.requested_pose_model.lower()
            if req in ("rtmw-x", "rtmw-l"):
                cfg.pose_model_tier = req
                cfg.vitpose_model_id = "usyd-community/vitpose-plus-base"
            elif req in ("vitpose-huge", "vitpose-large", "vitpose-base"):
                cfg.pose_model_tier = req
                model_map = {
                    "vitpose-huge": "usyd-community/vitpose-plus-huge",
                    "vitpose-large": "usyd-community/vitpose-plus-large",
                    "vitpose-base": "usyd-community/vitpose-plus-base",
                }
                cfg.vitpose_model_id = model_map[req]
            elif req.startswith("movenet"):
                cfg.pose_model_tier = "movenet"
                cfg.vitpose_model_id = "usyd-community/vitpose-plus-base"
                cfg.batch_size_video = 1
                cfg.fp16_enabled = False
